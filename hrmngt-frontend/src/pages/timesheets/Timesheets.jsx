...
import timesheetsData from '../../mock-data/timesheets';
import { useTimesheets, approveTimesheet, rejectTimesheet } from '../../hooks/useTimesheets';
...
function AdminTimesheetsView() {
  ...
  const handleApprove = (ts) => {
    const approved = approveTimesheet(ts.id, user ? `${user.firstName} ${user.lastName}` : 'HR Admin');
    if (approved) {
  ...
  const handleReject = (ts) => {
    const rejected = rejectTimesheet(ts.id);
    if (rejected) {
...
function EmployeeTimesheetsView() {
  ...
  const [selectedTimesheet, setSelectedTimesheet] = useState(null);
  const [isDetailOpen, setIsDetailOpen] = useState(false);
  const [records, setRecords] = useState(() => timesheetsData.filter(t => t.employeeId === employeeId));
  ...
  const handleSubmit = (ts) => {
    ...
    setRecords(prev => prev.map(r => (r.id === ts.id ? updated : r)));
