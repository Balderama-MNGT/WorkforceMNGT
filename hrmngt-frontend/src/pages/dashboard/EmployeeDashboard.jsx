...
export default function EmployeeDashboard() {
  ...
  const myLeaves = useMemo(
    () => leaves.filter((l) => l.employeeId === employeeId).sort((a, b) => b.appliedDate.localeCompare(a.appliedDate)),
    [employeeId]
  );

  const mySchedule = useMemo(
    () => shiftSchedules.filter((s) => s.employeeId === employeeId).slice(-7),
    [employeeId]
  );
  ...
  const leaveBalance = 15 - myLeaves.filter((l) => l.status === 'Approved').reduce((sum, l) => {
    const days = (new Date(l.endDate) - new Date(l.startDate)) / 86400000 + 1;
    return sum + days;
  }, 0);
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col h-[416px]">
          ...
          <div className="divide-y divide-gray-100/70 flex-1 overflow-y-auto overflow-x-hidden">
            ...
            {mySchedule.slice().reverse().map((entry) => {
              const shift = shiftDefinitions.find((s) => s.id === entry.shiftId);
              return (
